...
#                        → write <spec>/bootstrap.sh (creates target dir), generate prompt
#
# Usage:
...
# --- Bootstrap mode: generate bootstrap.sh ---
if [ "$GIT_MODE" = false ]; then
    BOOTSTRAP_FILE="$PROJECT_DIR/bootstrap.sh"
    BOOTSTRAP_DATE="$(date '+%Y-%m-%d %H:%M:%S')"
    cat > "$BOOTSTRAP_FILE" << BOOTSTRAP_EOF
#!/bin/bash
# Bootstrap: $PROJECT_NAME
# Auto-generated by bin/oneshot.sh on $BOOTSTRAP_DATE
...
BOOTSTRAP_EOF
    ...
    echo "  Bootstrap:  $BOOTSTRAP_FILE" >&2
    echo "" >&2
fi
