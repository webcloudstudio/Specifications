...
These are propagated to all set-up projects by `bash bin/update_projects.sh`.
Validated at PROTOTYPE level by `bin/validate_project.py`.
...
2. **Generate it** — in `bin/create_project.py` `create_project()`, call `copy_template('myfile', dest)`
3. **Propagate it** — in `bin/create_project.py` `update_projects()`, add to the loop (or add a similar `copy_template` call)
4. **Validate it** — add a rule name to `RULES_BY_LEVEL` in `bin/validate_project.py` and implement its `check()` case
5. **Document it** — add a row to the table above in this file

`bin/update_projects.sh` delegates to `create_project.py --update`, so step 3 is automatic once the code is in `update_projects()`.
