...
**Authoring phase vs. build phase — do not mix:**
- `REFERENCE_GAPS.md`, `SPEC_SCORECARD.md`, and `SPEC_ITERATION.md` are generated by `spec_iterate.sh` after the specification is stable enough to build. Do NOT create these files while actively authoring a specification.
- Numbered ticket files (`SCREEN-NNN-*`, `FEATURE-NNN-*`, `PATCH-NNN-*`, `AC-NNN-*`) are post-build iteration artifacts. Do NOT create them during specification authoring.
- During authoring, all unresolved questions belong in `## Open Questions` sections of the relevant spec file.
- Only run `spec_iterate.sh` or `iterate.sh` when the operator signals the specification is ready to build or iterate against an existing codebase.
