...
- **BDC-specific XBRL concepts require per-ticker discovery.** Standard us-gaap concepts cover NAV
  ...
  PIK % denominator (total investment income): discover per-ticker — TCPC confirmed `InvestmentIncomeNonoperating`;
  try `InvestmentIncomeNonoperating`, `GrossInvestmentIncomeOperating`, `InvestmentIncomeOperating` in order.
  NII per share: `InvestmentCompanyInvestmentIncomeLossPerShare` (confirmed TCPC Q1 2026: $0.22/share).
  Distributions per share: `CommonStockDividendsPerShareDeclared` for ARCC/TCPC; `InvestmentCompanyDistributionToShareholdersPerShare` for MFIC.
  Portfolio aggregate mark: `InvestmentOwnedAtFairValue` ÷ `InvestmentOwnedAtCost` from `companyfacts` (TCPC: 1 obs each = portfolio total). Per-position marks require inline XBRL from the filing HTML — **companyfacts has only aggregate totals for these concepts**.
  Asset coverage HTML regex (TCPC, no XBRL): `r"asset coverage ratio was (\d+\.?\d*)\s*%"` — do not use a broader pattern that captures the BDC reform text ("200% to 150%").
  Fee waiver: extract dollar amount from income statement line "Management fee waiver"; em dash (&#8212; / —) = $0 for the period. Do not use text-presence detection — the agreement language appears in every filing regardless of whether a waiver was applied.
...
- **Database feasibility** — RESOLVED: Six tables: `filings`, `metrics`, `positions`, `filing_flags`, `scores`, `eightk_events`. Six v1 bugs fixed: (1) position dedup key — XBRL SoI produces 3–5 duplicate rows per investee, now blocked by `UNIQUE(filing_id, dedup_key)`; (2) `positions.company_name_raw` preserves the raw concatenated XBRL label; parsed fields (`issuer_name`, `sector`, `instrument_type`) are NULL until a name-parsing pass; (3) `asset_coverage_pct` renamed from `asset_coverage_ratio` — v1 regex returned nonsense values (3.0, 8.0 instead of ~165%); (4) `scores` table separated from `metrics`; (5) `filing_flags` table replaces packed `extraction_notes` strings; (6) `realized_gain_loss` separated from `unrealized_depreciation` — v1 stored same value for both due to a shared XBRL concept. Schema loads and passes FK integrity. New findings from Step 5 spike: (a) ARCC UGL present (4 observations) but all carry `filed=2026-04-28` — sourced entirely from the Q1 2026 10-Q's comparative periods, not original quarterly filings; restatement drift undetectable without per-accession comparison; (b) MFIC UGL present and complete (4 recent observations, varying filed dates) — earlier "2022-vintage only" prediction was incorrect; (c) CONFIRMED: XBRL companyfacts stores multiple observations per `(period, form_type)` pair; "latest filed" selection is independent per concept; NAV and UGL source-filing dates diverge for 8 of 12 metric rows — the `filing_flags` table automatically surfaces these as `metric_source_mismatch` warnings; TCPC Q2 2025 PIK income gap confirmed (1 `data_gap` flag). Schema: `evidence/edgar2_spike.db`; load script: `build_schema.py` in build workspace. See `evidence/STEP_5_DATABASE_FEASIBILITY.md`.
- **Signal feasibility and scale direction** — RESOLVED pending PO confirmation of 7 open questions (thresholds, score direction, top-15 scope). Tested live against TCPC XBRL + HTML Q1 2026. **All 11 signals are extractable** — prior "2 not computable" claim is corrected. **Confirmed working (5):** (1) PIK % of total investment income — `InterestAndDividendIncomeOperatingPaidInKind` ÷ `InvestmentIncomeNonoperating`; TCPC Q1 2026 = 8.5%; (2) PIK % QoQ change (derived); (3) weighted avg portfolio mark — `InvestmentOwnedAtFairValue` ÷ `InvestmentOwnedAtCost` from companyfacts = 90.44% for TCPC Q1 2026; (4) unrealized change — XBRL quarterly delta = -$82.1M; (5) unrealized depreciation streak = 1 consecutive quarter. **Broken but fixable (4):** (a) NAV per share — `NetAssetValuePerShare` confirmed present, v1 used label search instead of dict key; (b) asset coverage ratio — correct regex `r"asset coverage ratio was (\d+\.?\d*)\s*%"` confirmed → 168.3%; v1 regex captured first digit group; (c) top-15 position marks — companyfacts holds aggregate totals only (1 row each); per-position inline XBRL in filing HTML has 638 cost + 726 FV rows — **recommend deferring to Phase 2** pending PO decision; (d) fee waiver — extract income statement amount (em dash = $0); TCPC Q1 2026 = $0, Q1 2025 comparative = $1,827,948. **Confirmed extractable, previously claimed "not computable" (2):** non-accrual — HTML prose regex, TCPC Q1 2026: 7.6% cost / 2.8% FV; NII coverage — `InvestmentCompanyInvestmentIncomeLossPerShare` ÷ `CommonStockDividendsPerShareDeclared`, TCPC Q1 2026 = 1.29x ($0.22 / $0.17). TCPC Q1 2026 theoretical score: 6–7 points (High band). Scale direction, band thresholds, NAV/non-accrual/PIK thresholds, and top-15 deferral pending PO confirmation — see `evidence/STEP_6_SIGNAL_FEASIBILITY_AND_SCALE_DIR.md`.
